...
type CaktoWebhook = {
  ...
  // Some providers send fields flat at top level
  id?: string;
  order_id?: string;
  product_id?: string;
  status?: string;
...
async function findUserByEmail(email: string | undefined | null): Promise<string | null> {
  ...
  // auth.users isn't queryable via supabase-js; use admin listUsers
  try {
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        // Resolve user id: prefer external_reference, fall back to email lookup
        let userId: string | null = ref?.userId ?? null;
        if (!userId) userId = await findUserByEmail(email);
        if (!userId) return new Response("ok"); // no-op: unknown user
        ...
        if (isRefundEvt) {
          // Cancel any active subscriptions for this user, revoke remaining credits.
          await supabaseAdmin
            .from("user_subscriptions")
